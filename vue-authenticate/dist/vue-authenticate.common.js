      ...
      try {
        ...
        if (popupWindowPath === redirectUriPath) {
          if (this$1.popup.location.search || this$1.popup.location.hash) {
            ...
            var hash$1 = parseQueryString(this$1.popup.location.hash.substring(1).replace(/[\/$]/, ''));
            var params = objectExtend({}, query);
            params = objectExtend(params, hash$1);
        ...
        if (crossWindowMsgFallback) {
          ...
          if (idxSearch > 0) {
            ...
            var query$1 = parseQueryString(search.substring(1).replace(/\/$/, ''));
            var hash = parseQueryString(hash.substring(1).replace(/[\/$]/, ''));
            var params$1 = objectExtend({}, query$1);
            params$1 = objectExtend(params$1, hash);

            // console.error('got params')
            // console.error(params)
            if (params$1.error) {
              reject(new Error(params$1.error));
            } else {
              resolve(params$1);
            }
    ...
    this$1.oauthPopup.open(this$1.providerConfig.redirectUri).then(function (response) {
      ...
      if (response.state && ('' + response.state !== '' + this$1.storage.getItem(stateName))) {
        if (response.state && ('' + response.state === '')) {
          if (window.localStorage.getItem('stateCheck') !== this$1.storage.getItem(stateName)) {
             return reject(new Error('State parameter value does not match original OAuth request state value see code.. See code.'))
          }
        } else {
          return reject(new Error('State parameter value does not match original OAuth request state value see code.'))
        }
      }
...
OAuth2.prototype.refresh = function refresh (userData) {
  var stateName = this.providerConfig.name + '_state';
  if (isFunction(this.providerConfig.state)) {
    this.storage.setItem(stateName, this.providerConfig.state());
  } else if (isString(this.providerConfig.state)) {
...
VueAuthenticate.prototype.isExpiring = function isExpiring () {
  ...
  try {
    var expiryInSeconds = parseInt(this.storage.getItem(this.tokenName + '_expires'));
    return (currentDate > expiryInSeconds)
  } catch (e) {
    return false
  }
};
...
VueAuthenticate.prototype.Refresh = function Refresh () {
  ...
  return new Promise$1(function (resolve, reject) {
    var provider = this$1.storage.getItem(this$1.tokenName + '_provider');
    var refresh_token = this$1.storage.getItem(this$1.tokenName + '_refresh_token');
    if (('' + provider) === 'undefined' || ('' + provider) === 'null' || provider === undefined || provider === null) {
      var tmptokenName = this$1.tokenName.replace('vueauth_token','  vueauth_access_token');
      provider = this$1.storage.getItem(tmptokenName + '_provider');
      provider = window.localStorage.getItem('vue-authenticate.vueauth_access_token_provider'); // TODO: Fix this
    }
    if (('' + refresh_token) === 'undefined' || ('' + refresh_token) === 'null' || refresh_token === undefined || refresh_token === null) {
      var tmptokenName = this$1.tokenName.replace('vueauth_token','  vueauth_access_token');
      refresh_token = this$1.storage.getItem(tmptokenName + '_provider');
      refresh_token = window.localStorage.getItem('vue-authenticate.vueauth_access_token_refresh_token');// TODO: Fix this
    }
    ...
    if (!window.vueAuthLastExecute) {
      ...
      delete window['vueAuthLastExecute'];
      return reject(new Error('Looping, due to injector...'))
    }
  })
};
/**
...
VueAuthenticate.prototype.setToken = function setToken (response) {
  ...
  if (response.expires_in) {
    ...
    seconds = seconds + response.expires_in - (response.expires_in * 20 / 100);
    this.storage.setItem(this.tokenName + '_expires', seconds);
  }
...
VueAuthenticate.prototype.logout = function logout (requestOptions) {
  ...
  this.storage.removeItem(this.tokenName + '_refresh_token');
  this.storage.removeItem(this.tokenName + '_expires');
  this.storage.removeItem(this.tokenName + '_provider');
  ...
  return new Promise$1(function (resolve, reject) {
    ...
    return providerInstance.init(userData).then(function (response) {
      ...
      if (this$1.isAuthenticated()) {
        this$1.storage.setItem(this$1.tokenName + '_provider', provider);
        return resolve(response)
      } else {
        return reject(new Error('Authentication failed'))
      }
