(function () {
  function install (Vue, options) {
    ...
    // GitHub API full url
    var apiUrl = 'https://api.bitbucket.org'
     ...
     */
    Vue.prototype.BitBucketAPI = {
      /**
       * Set application wide BitBucketAPI token value
       * @param {String} newToken The new token value
       * @example
       * // from anywhere in the global Vue scope
       * Vue.BitBucketAPI.setToken('your user token')
       *
       * // from a .vue component
       * this.BitBucketAPI.setToken('your user token')
       */
       ...
       * // -------------------------------------------------
       * // from anywhere in the global Vue scope
       * Vue.BitBucketAPI.get('/user/repos', {}, [this.myGitHubData, 'repositories'])
       * // from a .vue component
       * this.BitBucketAPI.get('/user/repos', {}, [this.myGitHubData, 'repositories'])
       *
       *
       * // ----------------------------------------------------------
       ...
       * // ----------------------------------------------------------
       * // from anywhere in the global Vue scope
       * Vue.BitBucketAPI.get('/user/repos', {}, (response) => {
       *   console.log('got response:', response)
       * })
       * // from a .vue component
       * this.BitBucketAPI.get('/user/repos', {}, (response) => {
       *   console.log('got response:', response)
       * })
       */
       ...
       * @example
       * // from anywhere in the global Vue scope
       * Vue.BitBucketAPI.post('/repos/OWNER_NAME/REPO_NAME/issues', {
       *   title: 'My new issues from vue-github-api'
       * }, (response) => { console.log('posted it!', response) })
       *
       * // from a .vue component
       * this.BitBucketAPI.post('/repos/OWNER_NAME/REPO_NAME/issues', {
       *   title: 'My new issues from vue-github-api'
       * }, (response) => { console.log('posted it!', response) })
       */
      ...
      delete: function (uri, fillIn, errorCb) {
        ...
        vv.$http.delete(
          ...
          this._updateStore('downloaded')
          console.log('delete response fillin?')
          if (typeof fillIn === 'function') {
            // sending back the full response
            fillIn(response)
          } else {
      ...
      /**
       * Register your application Vuex store to improve it with BitBucketAPI Vuex store module
       * @param  {Object} store Your application Vuex store
       * @example
       * // from within a .vue component
       * this.BitBucketAPI.registerStore(this.$store)
       */
      registerStore: function (store) {
        ...
        // registering BitBucketAPI Vuex module
        store.registerModule('BitBucketAPI', {
          state: {
            // is BitBucketAPI currently downloading?
            downloading: false,
